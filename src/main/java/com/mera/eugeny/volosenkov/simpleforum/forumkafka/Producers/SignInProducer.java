...
@Service
public class SignInProducer extends  ProducerBase{
    private static final Logger logger = (Logger) LoggerFactory.getLogger(SignInProducer.class);
    private static final String TOPIC = "auth_user";
    private static final String LOG_HEADER_TEMPLATE = String.format("#### -> %s message", SignInProducer.class.getName());
    public SignInProducer(@Qualifier("kafkaTemplateLongUser") KafkaTemplate<Long, User> kafkaTemplate) {
    ...
    public QueryResult send(String login, String password)
    {
        logger.info(String.format("%s -> Trying to authorizate  user with login %s", LOG_HEADER_TEMPLATE, login));
